...
Btw, my italicization of *knowing math* is an intentional problematizing because I want to mess the idea that *knowing math* is the technical prowess of calculation or proofing. Instead, we should be talking about knowing *enough* math; like having an ontological relationship with mathematical formalism, general principles. Otherwise we're risking an axiomatic blur that demands devs need a stronger STEM background or they aren't fit for the industry. 

I was exhausted by math in school because I never learned it in a way that connected it to my material reality. But I've since found that understanding the higher-order applicabilities of math to our business products can function in a very similar same way to the oft-desired need for senior engineers to go wide with their knowledge of the stack (regardless of specialization) so they can bear expertise on architecture and design. For example, why don't we desire for senior React developers to understand just enough graph theory to *see* component architectures as graphs and the implications of data pipelining and message passing in a graph (bonus points for being a student of Christopher Alexanders' semilattices); or just enough category theory to understand the legacy of  composibility of first class functions.  
