...
Btw, my italicization of *knowing math* is an intentional problematizing because I'd rather promote *knowing math* to software engineers as an ontological relationship with mathematical formalism, vs the technical prowess of calculation or proofing. As I've argued before, specificity is key for industry axioms which are built on falsehoods and create anxiety for less initiated developers or marginalized folks. 

When we say software engineers should know math, it's too high a demand to intimate devs should demonstrate acuity in producing sophisticated calculation. Understanding higher-order applicabilities of math to our business realms can function in the same way we desire senior engineers to be wide with their knowledge of the stack regardless of specialization so they can bear expertise on architecture. For example, can we hope for  senior React developers to understand just enough graph theory to *see* component architectures as graphs and the implications of data pipelining and message passing in a graph (Bonus points for being a student of Christopher Alexanders' semilattices); or just enough category theory to leverage the composibility of first class functions.  

I mean, once we sit down to write code, we can easily offload the fastidious implementation details; the few lines copied or recreated from existing libraries or proofs.
